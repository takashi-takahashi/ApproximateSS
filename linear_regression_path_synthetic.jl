...
@time ss_result = rvamp(
    A, y, λ, Normal(), ApproximateSS.Diagonal(), 
    dumping=0.8, t_max=50, debug=false, info=true, pw=0.5, tol=1.0e-10, intercept=intercept
);

@time ss_result_sa = rvamp(
        A, y, λ, Normal(), ApproximateSS.DiagonalRestricted(),
